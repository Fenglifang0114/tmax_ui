...
class OneFmaWgtRecPageState extends State<OneFmaWgtRecPage> {
  ...
  void initState() {
    ...
    _eventBus1 = eventBus.on<EventRespFormulaRecByPage>().listen((event) {
      if (mounted) {
    ...
    _eventbus3 = eventBus.on<EventRespDelAllFormulaWgtRec>().listen((event) {
      if (mounted) {
        _fetchDataFromBackend();
      }
    });
  ...
  Future<void> exportWgtRecords(String path) async {
    try {
      List<FmaRecFromDb> selectedRecords = [];
      for (var key in _selectedOrders.keys) {
        if (_selectedOrders[key] == true) {
          for (var item in _fmaRecsList) {
            if (item.header?.recordId == key) {
      ...
      List<List<dynamic>> csvData = buildCsvDataFromFmaRecs(selectedRecords);
  ...
  void _exportAllFormulaRecs() async {
    ...
    String? outputFile = await FilePicker.platform.saveFile(
      ...
      fileName: 'formula_wgt_records_${widget.fmaId}.csv',
    );
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            onPressed: totalItems == 0
                ? null
                : () {
                    _exportAllFormulaRecs();
                  },
            child: Text(
              localizedStrings.fExportAllRecordsBtn,
              style: Theme.of(context).textTheme.bodySmall!.apply(
                    color: Theme.of(context).colorScheme.onPrimary,
                  ),
              overflow: TextOverflow.ellipsis,
